...
class GetMatchesControllerTest {
    ...
    @Test
    void 팀_소속_멤버면_200과_경기_목록을_반환한다() throws Exception {
        List<MatchListItemResponseDto> response =
                List.of(MatchListItemResponseDto.builder().matchId(1L).opponent("서울 드래곤즈").build());

        when(matchService.getMatches(1L, USER_ID, null)).thenReturn(response);

        mockMvc.perform(get("/api/v1/teams/1/matches").with(authentication(authenticatedUser())))
    ...
    void 토큰_없이_요청하면_401_UNAUTHORIZED를_반환한다() throws Exception {
        mockMvc.perform(get("/api/v1/teams/1/matches"))
    ...
    void 존재하지_않는_팀이면_404_TEAM_NOT_FOUND를_반환한다() throws Exception {
        when(matchService.getMatches(999L, USER_ID, null))
                .thenThrow(new CustomException(TeamErrorCode.TEAM_NOT_FOUND));

        mockMvc.perform(get("/api/v1/teams/999/matches").with(authentication(authenticatedUser())))
    ...
    void 팀_소속이_아니면_403_FORBIDDEN을_반환한다() throws Exception {
        when(matchService.getMatches(1L, USER_ID, null))
