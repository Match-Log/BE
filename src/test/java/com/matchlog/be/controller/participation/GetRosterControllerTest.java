...
class GetRosterControllerTest {
    ...
    @Test
    void 팀_소속_멤버면_200과_로스터를_반환한다() throws Exception {
        List<RosterItemResponseDto> response =
                List.of(
                        RosterItemResponseDto.builder()
                                .participationId(1L)
                                .teamId(1L)
                                .playerId(9L)
                                .name("임준혁")
        ...
        when(participationService.getRoster(USER_ID, 1L)).thenReturn(response);

        mockMvc.perform(get("/api/v1/teams/1/players").with(authentication(authenticatedUser())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
                .andExpect(jsonPath("$[0].playerId", is(9)));
    }
    ...
    void 존재하지_않는_팀이면_404_TEAM_NOT_FOUND를_반환한다() throws Exception {
        when(participationService.getRoster(USER_ID, 999L))
                .thenThrow(new CustomException(TeamErrorCode.TEAM_NOT_FOUND));

        mockMvc.perform(get("/api/v1/teams/999/players").with(authentication(authenticatedUser())))
    ...
    void 팀_소속이_아니면_403_FORBIDDEN을_반환한다() throws Exception {
        when(participationService.getRoster(USER_ID, 1L))
