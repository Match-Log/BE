...
class GetTeamControllerTest {
    ...
    @Test
    void 팀_소속_멤버면_200과_팀_정보를_반환한다() throws Exception {
        ...
        when(teamService.getTeam(USER_ID, 1L)).thenReturn(response);

        mockMvc.perform(get("/api/v1/teams/1").with(authentication(authenticatedUser())))
    ...
    void 존재하지_않는_팀이면_404_TEAM_NOT_FOUND를_반환한다() throws Exception {
        when(teamService.getTeam(USER_ID, 999L))
                .thenThrow(new CustomException(TeamErrorCode.TEAM_NOT_FOUND));

        mockMvc.perform(get("/api/v1/teams/999").with(authentication(authenticatedUser())))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error.code", is("TEAM_NOT_FOUND")));
    }

    @Test
    void 팀_소속_멤버가_아니면_403_FORBIDDEN을_반환한다() throws Exception {
        when(teamService.getTeam(USER_ID, 1L))
